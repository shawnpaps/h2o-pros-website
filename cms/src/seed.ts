...
import { serviceCategories, services } from '../../client/src/data/services';
...
const run = async () => {
  ...
  console.log('Seeded Payload with client dummy content.');
  console.log(
    `Services: ${services.length}, locations: ${locations.length}, FAQs: ${faqs.length}, reviews: ${reviews.length}, gallery items: ${galleryItems.length}, service categories: ${serviceCategories.length}.`,
  );
};
